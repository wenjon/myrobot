...
> 规格是实现层面的文档。功能范围、用户场景、优先级见 `docs/superpowers/prd.md`。
...
- `src/tts.js` — 双后端语音合成：Edge 神经语音（`/api/tts`，逐字时间轴 + 预取 + 情绪基调）/ Web Speech 兜底，含 `softStop`（渐弱软停）。详见第 18 章。
...
| `MAX_TURNS` | `10` | 情景记忆滑动窗口大小（轮数），超过就丢老的 |
| `ENABLE_SUMMARY` | `1` | 是否启用摘要/反思。`0` = 关掉，纯滑动窗口 |
| `SUMMARY_TRIGGER_CHARS` | `1200` | 被裁掉的老历史累计超过这么多字符，就触发一次反思摘要 |
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `TTS_ENGINE` | `edge` | edge / web |
| `TTS_VOICE` | `zh-CN-XiaoyiNeural` | 中文共 14 个音色，跑 `scripts/tts_preview.py` 生成试听 |
| `TTS_RATE` | `+8%` | 语速，百分比 |
...
```
  用户发消息
      ...
      ▼
  ┌───────────────────────────────────┐
  │  build_messages() 组装上下文     │  ←── 读：画像 + 摘要 + 滑动窗口
  │  1. system prompt                   │       （检索器负责拼，不直接读字段）
  │  2. 用户画像（语义记忆）             │
  │  3. 会话摘要（被压缩的情景记忆）      │
  │  4. 最近 N 轮对话（滑动窗口）         │
  │  5. 本轮 user 消息                   │
  └───────────────┬───────────────────┘
    ...
    用户听到回复           │
                          ▼
                  ┌───────────────────────┐
                  │ 轮次结束 commit_turn()  │  ←── 写：history 加一轮
                  │ 1. 存入 history          │
                  │ 2. 超过窗口？裁到 dropped_buffer │
                  │ 3. maybe_summarize()？     │
                  │    ├ 未达阈值 → 结束         │
                  │    └ 达阈值 → 反思提炼 →    │
                  │         ├ 新摘要（覆盖旧的） │
                  │         └ 画像变更？→ 冲突检查│
                  │               ├ 不冲突 → 直接写入 │
                  │               └ 冲突 → pending_conflicts │
                  └───────────────┬───────────┘
                                  │
                                  ▼
                         下发 llm_done + profile_conflict
                                  │
                                  ▼
                         用户看到画像冲突卡 → 点确认/拒绝
                                  │
                                  ▼
                         收到 profile_resolve → 落盘
```

为什么要在 `llm_done` 之后才发冲突卡，而不是对话中间就发：
- 对话正在进行时弹卡会打断对话流，体验差
- 画像变更是后台事务，不影响当前轮回答
- 跟 `status` 消息不同，画像变更是需要用户操作的，放在一轮结束最自然
...
```
                     ...
                     └─────┬─────┘
                           ...
                           ▼
                     ┌───────────┐
                     │  待确认    │  ← pending_conflicts 里，等用户拍板
                     └─────┬─────┘
                    是     │     否
            ┌────────────┘     └────────────┐
            ▼                               ▼
     ┌───────────┐                   ┌───────────┐
     │  确认采纳   │ → 写入 profile →  │  拒绝保留   │
     └─────┬─────┘                   └─────┬─────┘
           │                               │
           └───────────────┬───────────────┘
                           ▼
...
- 待确认状态**不落盘**。重启后 pending 冲突丢失 = 保留旧值，这是安全默认。
- 不冲突的变更（旧值为空 / 旧值与新值完全一致）直接写入，不打扰用户。
- 冲突检测是**语义级**的，不是字符串全等：`'小王'` vs `'小王。'` 算一致。详见 `memory/profile.py`。
...
#### 能力边界：先摸清 Edge 到底放开了什么
动手前逐项实测，结论如下（这决定了整个实现路径）：

| 手段 | 结果 |
|---|---|
| `<break time="...">` 插停顿 | ❌ 直接 `NoAudioReceived` |
| `<prosody>` / `<emphasis>` 局部标签 | ❌ 同上 |
| 整段 SSML 包裹 | ❌ 标签被当正文念出来（22s 音频） |
| `<mstts:express-as>` 情感风格 | ❌ Azure 付费特性，免费端点不支持 |
| 请求级 `rate` | ✅ `-50%` ~ `+100%`（超过 +100% 饱和） |
| 请求级 `pitch` | ✅ 仅 `±NHz` 语法；`%`/`st`/`high` 全部 `ValueError`；约 `+100Hz` 饱和 |
| 请求级 `volume` | ✅ `±N%` |
| 标点影响停顿 | ✅ 逗号 +360ms、省略号 +375ms，但层级不可调 |

即：**每次请求只能带一组全局参数，没有任何句内控制手段**。

#### 于是怎么做出句内起伏
既然一次请求只能一个参数组，就把一句话拆成多段、逐段用不同参数合成，再在服务端拼回一个 mp3：

```
"各位观众晚上好，欢迎收看今天的新闻，我是主播小柚。"
        │  prosody.plan()
        ▼
  ┌ 各位观众晚上好，   rate=-2%  pitch=+12Hz vol=+6%  ← 起势
  ├ 欢迎收看今天的新闻， rate=+0%  pitch=+4Hz  vol=+0%  ← 平稳推进
  └ 我是主播小柚。     rate=-9%  pitch=-16Hz vol=-3%  ← 收束降调
        │  各段并发合成 → 裁尾 → 插静音帧 → 字节拼接 → 时间轴平移
        ▼
  单个 mp3 + 单条逐字时间轴（对前端完全透明）
```

- `backend/pipeline/prosody.py` — 韵律规划（产出段列表，纯计算、可单测）
- `backend/pipeline/mp3_utils.py` — 帧级工具（测时长 / 造静音 / 裁尾）
- `backend/pipeline/tts_edge.py` — 编排（并发合成 + 拼接 + 时间轴平移）

#### 韵律规则：抑扬顿挫的三个维度
1. **语调曲线（抑扬）**——句首起势稍高，句中平稳，句末降调收束；
   疑问句反过来必须上扬（`+22Hz`），否则听着像自言自语；感叹句加强。
2. **轻重对比（挫）**——句首语气词（哇/啊/唉…）剥成独立段并拖长（`rate-18%`、`pitch+20Hz`），
   这是口语生动感最强的抓手；命中强调词（特别/超级/真的/太…）的段落额外加重放慢。
   句中段落还做奇偶交替微起伏，避免「每段都一样」的平板感。
3. **停顿层级（顿）**——`、`90ms < `，`130ms < `：`190ms < `；`210ms < `。`260ms < `…`320ms。
   层级差距刻意拉开；用精确长度的静音帧补齐，而非依赖 Edge 自带的标点停顿。

另外 `[表情:x]` 会作为**情绪基调**叠加成语速/音高/音量偏置
（开心 `+6%/+12Hz/+6%`，悲伤 `-10%/-14Hz/-6%`，困倦 `-16%/-12Hz/-10%`……）。
于是表情不只改脸，声音也跟着变，视听一致。`natural` 风格取一半强度，`flat` 关闭句内起伏。

#### 为什么可以直接按字节拼 mp3
Edge 返回的是 **MPEG-2 Layer III / 24kHz / 48kbps / 单声道**裸流
（帧头 `FF F3 64 C4`，帧长 144 字节 = 24ms），无 ID3、无 VBR 头，每帧可独立解码。
因此帧序列首尾相接即为合法 mp3，实测拼接后重新解析，帧数与总时长严格等于各段之和。
静音帧 = 复用首帧帧头 + 数据体填 0，粒度 24ms，足够精细。

#### 三个踩过的坑（已固化进 `scripts/check_tts.py`）
1. **MPEG-1 与 MPEG-2 的 Layer III 码率表不同**，每帧采样数也不同（1152 vs 576）。
   初版只用 MPEG-1 表，把 48kbps 误读成 80kbps → 帧长算大 → 跳过真帧、误判假帧
   → 时长少算约 20%（1104ms 实为 2184ms）→ 拼接平移量不足 → **时间轴回退、口型越走越慢**。
   另补帧长校验：按算出的帧长跳过去必须仍是同步字，否则跳过（音频数据里偶然出现的
   `FF Ex` 会被误认成帧头）。
2. **Edge 每段尾部附加约 0.7~0.9s 固定静音**。单段无感，多段拼接会累积
   （3 段句子凭空多出 2.4s）。解决：按「最后一个 WordBoundary + 60ms 余量」整帧裁掉。
   `flat` 模式也裁——否则句间白等近一秒。
3. **串行合成让延迟随段数线性增长**（3 段 ~5s）。改 `asyncio.gather` 并发后，
   3 段耗时回落到 ~1.9s，与单段（~2.0s）基本持平，**韵律不再是延迟负担**。

#### 实测数据
| 句子 | 无韵律 | 播音腔 |
|---|---|---|
| 各位观众晚上好，欢迎收看今天的新闻，我是主播小柚。 | 1 段 / 5016ms / 2.01s | 3 段 / 4968ms / 2.03s |
| 哇，这个也太厉害了吧！ | 1 段 / 2856ms / 1.56s | 2 段 / 2304ms / 1.42s |
| 我有点累了，想歇一会儿。 | 1 段 / 2592ms / 2.22s | 2 段 / 2712ms / 1.54s |

（格式：段数 / 音频时长 / 合成耗时）

#### 怎么听、怎么调
- `python scripts/tts_preview.py` → `voice_preview/` 下生成 9 个音色样本
  + 5 组 `P*_A_flat.mp3` / `P*_B_broadcast.mp3` 成对 A/B 样本。
- 页面 `#exprTest` 面板有两个下拉框：音色、韵律风格，可现场切换即时对比。
- 想更「播音」可加大 `prosody.py` 里首末段的 pitch 差与停顿值；
  想更自然就用 `TTS_PROSODY=natural`。



---

## 19. 摄像头感知与注视跟随（P2，设计阶段）

### 19.1 目标
双目摄像头检测到有人靠近时，数字人用眼睛 + 转头看向对方，距离越近反应越强烈。
体现"有人走过来 → 注意到 → 注视 → 靠近时打招呼"的自然流程。

**不在本期范围**：人脸识别、情绪识别、姿态估计。只有"是否有人 + 大致方位 + 距离"。

### 19.2 为什么放后端处理而不是浏览器
- 浏览器拿不到双目深度信息
- 视觉检测（OpenCV / MediaPipe / YOLO）需要 Python 环境
- 未来换 RealSense / 奥比中光等深度相机，也在 Python 侧适配更方便

### 19.3 架构

```
双目摄像头 ──▶ 视觉进程（Python） ── socket订阅 ──▶ backend/vision/subscriber.py
                                                │ 坐标映射 + 平滑 + 状态机
                                                ▼
                                       WS 广播 presence 消息
                                                │
...
- 视觉进程独立于 myrobot 后端，可以单独启停、单独重启
- 订阅协议待确认（ZeroMQ PUB/SUB / MQTT / 原生 TCP）
- myrobot 后端做一对多广播，所有连上的浏览器都收到 presence 消息
...
```json
{
  "type": "presence",
  "persons": [
    {"id": 1, "angle_x": -0.35, "angle_y": 0.1, "distance_m": 2.4, "conf": 0.92}
  ],
  "ts": 1730000000.123
}
```

- `angle_x/y` 归一化 `[-1, 1]`，直接喂给 `lookAt()`
- `distance_m` 是双目优势 —— 没有深度就无法做分级反应
- `id` 用于跨帧跟踪，避免多人时目标乱跳
- 空数组 = 画面没人（比不发消息更可靠）

### 19.5 行为状态机（关键）
机械跟随会很假。真人被靠近时是分级反应的：

| 状态 | 触发条件 | 反应 |
|---|---|---|
| idle（空闲） | 无人 / 很远（>4m） | 随机扫视 + 微摆 |
| glance（瞥一眼） | 首次检测到人（3~4m） | 只有眼睛扫过去，头不动 |
| track（注视） | 人进入 1.5~3m | 眼睛 + 转头跟随，表情切 `期待` |
| engage（互动） | 人进入 <1.5m | 加 `开心`/`惊讶` 表情，可选主动打招呼 |
| leave（目送） | 人离开视野 | 保持注视 1~2 秒后缓慢回落 |

状态切换需要：
- 低通滤波（目标位置不抖）
- 死区阈值（小范围不动）
- 冷却时间（目标切换要有延迟，不能左右乱甩）

### 19.6 待确认

1. **订阅协议**：ZeroMQ / MQTT / 原生 TCP 换行分隔？
2. **连接方向**：摄像头端监听（推荐）还是 myrobot 监听？
3. **数据字段**：方位角+距离 / 像素+深度 / 只有无人+左右？
4. **实现优先级**：先做协议+前端跟随+鼠标模拟，还是等摄像头调通再做？

### 19.7 开发顺序（建议）
1. 先定消息契约（本章 19.4）
2. 前端：新增 `presence` 消息处理 + 状态机
